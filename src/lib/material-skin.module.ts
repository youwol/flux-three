...
import {Property, Flux,BuilderView, ModuleFlux, Pipe, Schema, expect, Context, expectAnyOf, expectInstanceOf, expectAllOf } from "@youwol/flux-core"
import {MeshStandardMaterial, FrontSide, DoubleSide, BackSide, Color, Mesh, Group} from 'three'
import { createFluxThreeObject3D } from "./buffer-geometry.module"
 ...
 * ## Presentation
 * 
 * Apply a [standard material](https://threejs.org/docs/#api/en/materials/MeshStandardMaterial) on provided Mesh.
 *   
 *  ## Resources
 *
 * Various resources:
 ...
 * the material representation for three.js
  */
export namespace ModuleStandardMaterialSkin {
    ...
    export class PersistentData{
        ...
        @Property({
            description:"Object3D part"
        })
        object3D: Schemas.Object3DConfiguration = new Schemas.Object3DConfiguration({})

        @Property({
            description:"Material part"
        })
        material:  Schemas.StandardMaterialConfiguration = new Schemas.StandardMaterialConfiguration({})
        ...
        constructor( {object3D, material}: {
            ...
            material?:any} = {}
                ...
                if(object3D)
                    this.object3D = new Schemas.Object3DConfiguration(object3D)
                if(material)
                    this.material = new Schemas.StandardMaterialConfiguration(material)
        }
    }

    let contract =  expectAnyOf({
        description: `A mesh, array of meshes or a group of 3D objects containing meshes`,
        when: [
            expectInstanceOf({
                typeName: 'Mesh',
                Type: Mesh, attNames:['mesh']
            }),
            expectAllOf({
                description: `A group of 3D objects containing meshes`,
                when: [
                    expectInstanceOf({ typeName: 'Group', Type: Group }),
                    expect({
                        description: `The group contains meshes`,
                        when: (group: Group) => group.children.find(child => child instanceof Mesh) != undefined,
                        normalizeTo: (group: Group) => group.children.filter(child => child instanceof Mesh)
                    })
                ],
                normalizeTo: (accData: [Group, typeof Mesh[]]) => accData[1]
            }),
            expectAllOf({
                description: `An array containing meshes`,
                when: [
                    expectInstanceOf({ typeName: 'Array', Type: Array }),
                    expect({
                        description: `The array contains meshes`,
                        when: (objects: any[]) => objects.find(child => child instanceof Mesh) != undefined,
                        normalizeTo: (objects: any[]) => objects.filter(child => child instanceof Mesh)
                    })
                ],
                normalizeTo: (accData: [Group, typeof Mesh[]]) => accData[1]
            })
        ],
        normalizeTo: (data: typeof Mesh | typeof Mesh[]) => {
    ...
    @Flux({
        pack:           pack,
        namespace:      ModuleStandardMaterialSkin,
        id:             "ModuleStandardMaterialSkin",
        displayName:    "MaterialSkin",
        description:    "Standard material to apply on 3D objects",
        resources: {
            'technical doc': `${pack.urlCDN}/dist/docs/modules/lib_standard_material_skin_module.modulestandardmaterialskin.html`
        }
    })
    @BuilderView({
        namespace:      ModuleStandardMaterialSkin,
        ...
        constructor(params){ 
            ...
            this.addInput({
                ...
                onTriggered: ( {data, configuration, context}:{data:Mesh[], configuration: PersistentData, context: Context}) => {
                    this.process(data, configuration, context)
                }
            })
